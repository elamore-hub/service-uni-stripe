...
export class EventUsesComponent implements OnInit, OnDestroy {
  ...
  constructor(
    protected eventUsesService: EventUsesService,
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.eventUsesService
      ...
      .subscribe(
        (res: HttpResponse<IEventUses[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInEventUses(): void {
    this.eventSubscriber = this.eventManager.subscribe('eventUsesListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IEventUses[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/event-uses'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
